...
/**
 * Rewrites absolute file:/// links to relative paths based on project markers.
 * Fallback mechanism for local paths generated by AI agents.
 */
function processRelativeLinks(text) {
  if (!text) return text;
  
  // Use the local secure proxy to fetch images, supporting private repos via GITHUB_TOKEN
  const PROXY_BASE = '/.netlify/functions/github?path=';
  
  // Regex to find file:/// URLs until the end of a markdown link or space
  return text.replace(/file:\/\/\/[^)\s]*/g, (url) => {
    // Look for repository markers to determine the relative starting point
    const markers = ['/packages/', '/projects/'];
    for (const marker of markers) {
      const idx = url.indexOf(marker);
      if (idx !== -1) {
        // Construct the proxy URL
        const relativePath = url.substring(idx + 1);
...
export function extractMetadata(content) {
  ...
  // Pre-process content to fix absolute image links
  const normalizedContent = processRelativeLinks(content);
  
  let stripped = normalizedContent;
  let mdMeta = [];
  let isMarp = false;
  ...
  const rawContentForMarp = normalizedContent;
