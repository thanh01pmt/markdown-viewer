// packages/interactive-quiz-kit/src/ai/flows/question-gen/generate-mcq-question.ts
'use client';

import { z } from 'zod';
import { GoogleGenerativeAI, Part } from '@google/generative-ai';
import type { MultipleChoiceQuestion, QuestionOption } from '../../../types';
import { generateUniqueId } from '../../../utils/idGenerators';
import {
  type GenerateMCQQuestionClientInput,
  type GenerateMCQQuestionOutput,
  AIMCQOutputFieldsSchema,
} from './generate-mcq-question-types';
import { MultipleChoiceQuestionZodSchema } from './question-generation-schemas';
...
function buildEnhancedPrompt(clientInput: GenerateMCQQuestionClientInput, attemptNumber: number): string {
  const { quizContext, language, difficultyCode, numberOfOptions, imageUrl } = clientInput;
...
export async function generateMCQQuestion(
  clientInput: GenerateMCQQuestionClientInput,
  apiKey: string
): Promise<GenerateMCQQuestionOutput> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const completeQuestion: MultipleChoiceQuestion = {
  ...
  const errorMessage = `Failed to generate MCQ question after ${MAX_RETRY_ATTEMPTS} attempts. Last error: ${lastError?.message}`;
  console.error('\n❌ Final Result: FAILED');
