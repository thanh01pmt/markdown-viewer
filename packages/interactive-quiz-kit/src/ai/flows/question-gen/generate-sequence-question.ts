// packages/interactive-quiz-kit/src/ai/flows/question-gen/generate-sequence-question.ts
'use client';

import { z } from 'zod';
import { GoogleGenerativeAI, Part } from '@google/generative-ai';
import type { SequenceQuestion, SequenceItem } from '../../../types';
...
import {
  type GenerateSequenceQuestionClientInput,
  type GenerateSequenceQuestionOutput,
  AISequenceOutputFieldsSchema,
} from './generate-sequence-question-types';
import { SequenceQuestionZodSchema } from './question-generation-schemas';

const MAX_RETRY_ATTEMPTS = 3;
const RETRY_DELAY_MS = 3000;

function buildEnhancedPrompt(clientInput: GenerateSequenceQuestionClientInput, attemptNumber: number): string {
  const { quizContext, language, difficultyCode, numberOfItems, imageUrl } = clientInput;
  ...
  const imageContextInstruction = imageUrl
    ? `**Image Context:** You MUST analyze the provided image. The sequence of items must be directly related to the process or content shown in this image.`
    : '';
...
export async function generateSequenceQuestion(
  clientInput: GenerateSequenceQuestionClientInput,
  apiKey: string
): Promise<GenerateSequenceQuestionOutput> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const finalItems: SequenceItem[] = [];
      const finalCorrectOrder: string[] = [];
      ...
      const completeQuestion: SequenceQuestion = {
  ...
  const errorMessage = `Failed to generate Sequence question after ${MAX_RETRY_ATTEMPTS} attempts. Last error: ${lastError?.message}`;
  console.error('\n❌ Final Result: FAILED');
